  ...
  <div class="flex md:flex-row flex-col items-center justify-center w-full h-full md:space-x-4 md:space-y-0 space-y-4 ">
            ...
            <div class="w-full bg-white border-4 border-black rounded-xl p-6">
               ...
               <div class="flex justify-between items-center pb-4">
                  ...
                  <!-- Right: Total Price -->
                  <div
                     class="py-3 px-6 border-2 border-red-600 rounded-xl bg-red-400 shadow-lg text-center"
                     >
                     <h2 class="text-lg font-extrabold text-black uppercase">Total Retail Price:
                        <span class="text-xl font-bold text-black">
                        {{ totalRetailPrice.toLocaleString() }} LKR
                        </span>
                     </h2>
                  </div>
                  <div
                     ...
                     >
                     <h2 class="text-lg font-extrabold text-black uppercase">Total Wholesale Price:
                        <span class="text-xl font-bold text-black">
                        {{ totalWholesalePrice.toLocaleString() }} LKR
                        </span>
                     </h2>
                  </div>
               </div>
                  ...
                  <table
                     ...
                     <thead>
                        <tr class="bg-gradient-to-r from-blue-700 via-blue-600 to-blue-700 text-white text-[14px] border-b border-blue-800">
                           ...
                           <th class="p-3 text-center font-semibold">Retail Info</th>
                           <th class="p-3 text-center font-semibold">Total Retail Price</th>
                           <th class="p-3 text-center font-semibold">Wholesale Info</th>
                           <th class="p-3 text-center font-semibold">Total Wholesale Price</th>
                           <th class="p-3 text-center font-semibold">Total Wholesale Price</th>
                        </tr>
                     </thead>
                     <tbody class="text-[12px] font-medium">
                        <tr
                           ...
                           <td class="p-3 text-left leading-6 text-sm">
                              <div><strong>Price:</strong> {{ product.selling_price || "N/A" }} LKR</div>
                              <div><strong>Profit:</strong> {{ (product.selling_price - product.cost_price).toFixed(2) || 0 }} LKR</div>
                           ...
                           <td class="p-3 text-center">
                              {{ (product.sales_qty * getRetailPrice(product)).toFixed(2) }} LKR
                           </td>
                           <td class="p-3 text-center">
                              {{ (product.sales_qty * getRetailPrice(product)).toFixed(2) }} LKR
                           </td>
                           <!-- Wholesale Info -->
                           <td class="p-3 text-left leading-6 text-sm">
                              <div><strong>Whole Price:</strong> {{ product.whole_price || "N/A" }} LKR</div>
                              <div><strong>Discount:</strong> {{ product.wholesale_discount || 0 }}%</div>
                              <div>
                                 <strong>Final:</strong>
                                 {{
                                 product.wholesale_discount <= 100
                                 ? (product.whole_price * (1 - product.wholesale_discount / 100)).toFixed(2)
                                 : (product.whole_price - product.wholesale_discount).toFixed(2)
                                 }} LKR
                              </div>
                           </td>
                           ...
                           <td class="p-3 text-center">
                              ...
                              (
                              product.wholesale_discount <= 100
                              ? (product.whole_price * (1 - product.wholesale_discount / 100))
                              : (product.whole_price - product.wholesale_discount)
                              )
                              ).toFixed(2)
                              }} LKR
                           </td>
...
<script setup>
   ...
   const totalPrice = computed(() => {
     return (parseFloat(totalRetailPrice.value) + parseFloat(totalWholesalePrice.value)).toFixed(2);
   });
   ...
   const totalRetailPrice = computed(() => {
     return products.value.reduce((sum, product) => {
       const retailPrice = product.discount <= 100
         ? (product.selling_price * (1 - product.discount / 100))
         : (product.selling_price - product.discount);
       return sum + (product.sales_qty * retailPrice || 0);
     }, 0).toFixed(2);
   });

   const totalWholesalePrice = computed(() => {
     return products.value.reduce((sum, product) => {
       const wholePrice = product.wholesale_discount <= 100
         ? (product.whole_price * (1 - product.wholesale_discount / 100))
         : (product.whole_price - product.wholesale_discount);
       return sum + (product.sales_qty * wholePrice || 0);
     }, 0).toFixed(2);
   });


   // const totalPrice = computed(() => {
   //   return totalRetailPrice.value + totalWholesalePrice.value;
   // });
   ...
   const downloadPDFTable = () => {
     ...
     // Table headers matching your frontend
     const tableColumn = [
       "#",
       "Name",
       "QTY",
     ...
     const tableRows = products.value.map((product, index) => {
       ...
       const retailPrice = discount <= 100
         ? sellingPrice * (1 - discount / 100)
         : sellingPrice - discount;

       const salesQty = Number(product.sales_qty || 0);

       const wholePrice = Number(product.whole_price || 0);
       const wholeDiscount = Number(product.wholesale_discount || 0);
       const finalWholesalePrice = wholeDiscount <= 100
         ? wholePrice * (1 - wholeDiscount / 100)
         : wholePrice - wholeDiscount;

       const totalRetail = salesQty * retailPrice;
       const totalWholesale = salesQty * finalWholesalePrice;
       ...
       return [
         ...
         finalWholesalePrice.toFixed(2),
         totalWholesale.toFixed(2),
       ];
     });

     // Calculate grand totals
     const totalRetailSum = tableRows.reduce((sum, row) => sum + parseFloat(row[6]), 0);
     const totalWholesaleSum = tableRows.reduce((sum, row) => sum + parseFloat(row[8]), 0);
     ...
     tableRows.push([
       ...
       "",
       "",
       totalRetailSum.toFixed(2),
       "",
       totalWholesaleSum.toFixed(2),
     ]);
       ...
       columnDefs: [
         {
           ...
           searchable: false,
            // Disable sorting for this specific column
         },
       ],
